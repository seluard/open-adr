...
export default function AdrBrowser({ defaultPublic = false, initialOwner, initialRepo }: { defaultPublic?: boolean; initialOwner?: string; initialRepo?: string }) {
  ...
  // Preselect repository if provided via props
  useEffect(() => {
    if (!selectedRepo && initialOwner && initialRepo) {
      setSelectedRepo({
        id: `${initialOwner}/${initialRepo}`,
        name: initialRepo,
        owner: { login: initialOwner }
      });
    }
  }, [initialOwner, initialRepo, selectedRepo]);
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <Sidebar onSelectRepo={setSelectedRepo} defaultPublic={defaultPublic} selectedRepo={selectedRepo} />
        ...
        <div className="w-1/3 max-w-xs min-w-[220px] p-6 overflow-y-auto" style={{ background: undefined, color: undefined }}>
          ...
          {selectedRepo ? (
            <>
              ...
              {!loading && listError && (
                <p className="text-red-600 text-sm mb-2">{listError}</p>
              )}

	...
	<nav className="flex items-center justify-between px-5 py-2 shadow-md" style={{ boxShadow: '0 4px 16px -4px rgba(0,0,0,0.15)' }}>
			...
			<div className="flex items-center gap-3">
				...
				<div className="hidden sm:flex items-center gap-2 text-sm text-gray-700">
					<span className="font-medium">{repoInfo?.full_name || `${owner}/${repo}`}</span>
					<a
						...
						className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200 hover:bg-yellow-100 hover:border-yellow-300 no-underline hover:no-underline focus:no-underline"
					>
						...
						<span className="no-underline">{typeof repoInfo?.stargazers_count === 'number' ? repoInfo.stargazers_count : '—'}</span>
					</a>
				</div>
				{session ? (
